    ...
    try:
        ...
        if 'message' in body:
            ...
            if 'text' in message:
                ...
                #dynamo.update_item('wokemon_users', 'telegram_id', from_id, 'telegram_username', from_username)
...
def bot_command_raid(command_params, chat_id):
    ...
    raid_dict = raid.get_raid_by_id(command_params)
    if not raid_dict:
        return msg.send_message('⚠️ Raid with raid_id={0} not found.'.format(command_params), chat_id)
    
    tracked_message = msg.send_message(raid.format_raid_message(raid_dict), chat_id, 'MarkdownV2', True)
...
def bot_command_nickname(command_params, chat_id, from_id, from_username):
    ...
    try:
        if dynamo.update_item('wokemon_users', 'telegram_id', from_id, 'trainer_name', command_params):
...
def bot_command_team(command_params, chat_id, from_id, from_username):
    ...
    try:
        if dynamo.update_item('wokemon_users', 'telegram_id', from_id, 'trainer_team', command_params.title()):
...
def bot_command_level(command_params, chat_id, from_id, from_username):
    ...
    if not str(command_params).isdigit():
        ...
        try:
            if dynamo.update_item('wokemon_users', 'telegram_id', from_id, 'trainer_level', level):
                if level == 50:

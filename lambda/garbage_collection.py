...
def garbage_collection(client):
    
    # First lets get a list of raids that have not been completed
    active_raids = dynamo.scan_attribute_not_exists('wokemon_raids', 'completed', client)
    ...
    for r in active_raids:
        ...
        if (raid_datetime + timedelta(minutes = 10)) < datetime.now():
            dynamo.update_item('wokemon_raids', 'raid_id', r.get('raid_id'), 'completed', True, client)
            reply.update_tracked_messages(r.get('raid_id'), client, allow_delete=True)
